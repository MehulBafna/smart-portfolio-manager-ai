"""
Stock Analysis Page — deep dive into individual stocks with
price charts, technicals, fundamentals, news, and AI signal.
"""
...
def render():
    ...
    if st.button(f"Generate AI Signal for {name}", type="primary", use_container_width=True):
        with st.spinner("Analyzing news, technicals, fundamentals... (30-60s)"):
            rag_result = cached_rag_context(ticker, name)
            signal = analyze_stock(
                ticker, name, tech, fundamentals,
                rag_result["context_text"], holding
            )
    ...
    if f"signal_{ticker}" in st.session_state:
        ...
        col_target.metric("Target Price", f"₹{sig.target_price:,.0f}" if sig.target_price else "N/A",
                          f"{sig.upside_potential_pct:+.1f}%" if sig.upside_potential_pct else "")
        col_horizon.metric("Time Horizon", sig.time_horizon)
        col_risk.metric("Risk Level", sig.risk_level)
        ...
        st.caption(f"Technical: {sig.technical_summary} | Fundamental: {sig.fundamental_summary} | News: {sig.news_sentiment} sentiment")
